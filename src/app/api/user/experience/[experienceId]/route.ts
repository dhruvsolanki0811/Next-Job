...
export async function PUT(req: NextRequest, context: { params: Params }) {
  try {
    ...
    return NextResponse.json({ experience: updatedExperience }, { status: 200 });
  } catch (err) {
    console.log(err)
...
export async function DELETE(req: NextRequest, context: { params: Params }) {
    try {
        ...
        const deleteExperience = await db.experience.delete({
          ...
          return NextResponse.json({ experience:deleteExperience,message:"Deleted Successfully!" }, { status: 200 });
    }catch(err){
        return NextResponse.json({ error: err }, { status: 500 });
    }    
}
